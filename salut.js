...
var AllEvents = {
  ...
  changearea: function(area) {
    ...
    if (area) {
      ...
      if (CurrentJaaJWorld.player && typeof bluePrint !== 'undefined') {
...
function getQueryString(obj) {
  ...
  for (var key in obj) {
    if (obj.hasOwnProperty(key)) {
      QueryString += QueryString == '?' ? key+'='+obj[key] : '&'+key+'='+obj[key];
    }
  }
  return QueryString;
};
...
BluePrint.prototype = {
  ...
  load: function(name,area,b) {
    ...
    editor.gettmp({
      data: JSON.stringify([{name:this.area+'_BluePrint_'+this.name+'.json',JaaJWorld:CurrentJaaJWorld.name}]),
      onend: function(data) {
...
Animation.prototype.load = function (id,c) {
  ...
  this.current[id].image.src = 'assets/map/'+CurrentJaaJWorld.name+'/animation/'+this.current[id].src;
};
...
function Player(settings) {
  ...
  this.skin.src = 'assets/map/'+CurrentJaaJWorld.name+'/mob/player/skin/'+(settings.skin || 'default')+'.png';
  this.spriteSheet = {
    posx: 0,
    posy: 0,
    dx: 64,
  ...
  this.randomMoveTime = Date.now();
  this.randomMove = 'stop';
};
Player.prototype = {
    ...
    if (!this.boxCollision.collision.b) {
      ...
      if (CurrentJaaJWorld.player) {
        ...
        if (this.nextAction != 'stop') {
          ...
          if (Date.now() - this.randomMoveTime > getRandomIntInclusive(1000, 5000)) {
            this.randomMoveTime = Date.now();
            var moveArray = ['up','left','down','right','stop'];
            this.randomMove = moveArray[getRandomIntInclusive(0, moveArray.length-1)];
          }
        }
      }
  ...
  active: function(a,b) {
    ...
    if (!a) {
      ...
      var xy = world.getRelativeXY();
      var oldxy = xy;
      if (!b) window.onchangearea();
    } else {
      ui.hide();
      $('body').css('background','#000');
      var xy = world.getRelativeXY();
      var oldxy = xy;
      if (!b) window.onchangearea('['+xy.x+','+xy.y+']'+world.currentMap);
    }
  },
  getData: function() {
    return {
...
Mob.prototype = {
    ...
    socket.SetMysql(d,function(data){
      if (data.length > 0) {
        ...
        socket.SetMysql(d,function(datac){
          ...
          _this.spriteSheet.src = 'assets/map/'+CurrentJaaJWorld.name+'/mob/'+MobData_Constructor.spriteSheet;
        });
      }
    });
  },
  ...
  del: function() {
    ...
    ui.NeedValid(function(){
      ...
      socket.SetMysql(d,function(data){
        if (data.success) {
          socket.sk.emit('JAAJeditorDeleteFile','assets/map/'+CurrentJaaJWorld.name+'/editor/Entity_BluePrint_'+_this.id+'-'+_this.name+'_MOB.json');
        }
      });
      for (var i in world.ObjectMob) {
        if (world.ObjectMob.hasOwnProperty(i)) {
  ...
  if (player.isLoaded) {
    ...
    for (var i = 0; i < memory.length; i++) {
      ...
      if (typeof this.temp[i] === 'undefined') {
        ...
        this.temp[i].src = 'assets/map/'+CurrentJaaJWorld.name+'/jaajs/'+jaajList[id][jaaj.evolutionLvl || 0].imagePath;
...
Menu.prototype.drawItem = function (itemID,Nb,x,y) {
  ...
  if (typeof this.Ressources[itemID] === 'undefined') {
    ...
    this.Ressources[itemID].src = 'assets/map/'+CurrentJaaJWorld.name+'/item/'+currentItem.Image;
  }
  var size = 130;
  ctx.save();
...
function SystemFight(a,b,c,d,e) {
  ...
  this.background.src = 'assets/map/'+CurrentJaaJWorld.name+'/battle/'+d[0]+'/'+world.getTimeData()+'_'+d[1]+'.jpg';
...
SystemFight.prototype.updateJaaj = function (index,exp,c) {
  ...
  if (typeof this.a.jaajs[index] !== 'undefined') {
    if (this.a.jaajs[index].fight) {
      ...
      this.v1Reward = -300;
      this.ImageReward.OK = false;
      this.ImageReward.src = 'assets/map/'+CurrentJaaJWorld.name+'/jaajs/'+jaajList[this.a.jaajs[index].id][this.a.jaajs[index].evolutionLvl || 0].imagePath;
...
FightTrainer.prototype.sendJaaj = function (index) {
  ...
  this.current.image.src = 'assets/map/'+CurrentJaaJWorld.name+'/jaajs/'+jaajList[id][this.jaajs[index].evolutionLvl || 0].imagePath;
    ...
    this.memoryObject = this.memoryObject[this.id];
      ...
      if (this.isC) {
        if (refMemoryMob) {
          ...
          socket.SetMysql2(d,function(data){next();});
        } else {
          var d = {
            mode: 'UPDATE',
            table: 'editormembre',
...
function Resource() {
  ...
  for (var color in this.Cursors) {
    if (this.Cursors.hasOwnProperty(color)) {
      this.Cursors[color].image.src = 'assets/map/'+CurrentJaaJWorld.name+'/editor/cursor/'+color+'.png';
      (function(color,obj){
...
Resource.prototype = {
  getJaajs: function(callback) {
    ...
    $.ajax({
      type: "POST",
      url: '/jaajCommand',
      data: 'getJaajs:'+CurrentJaaJWorld.name,
      processData: false,
      contentType: false,
  ...
  setSpriteSheet: function(filename,data) {
    ...
    this.currentSpriteSheetURI = 'assets/map/'+CurrentJaaJWorld.name+'/mob/'+filename;
  ...
  getSpriteSheets: function(callback) {
    ...
    $.ajax({
      type: "POST",
      url: '/jaajCommand',
      data: 'getSpriteSheets:'+CurrentJaaJWorld.name,
      processData: false,
      contentType: false,
      success: function(data) {
        if (!data) {
  ...
  setAnimationSpriteSheet: function(filename,data) {
    ...
    data = data || {};
    this.currentSpriteSheetNAME = filename;
    this.currentSpriteSheetURI = 'assets/map/'+CurrentJaaJWorld.name+'/animation/'+filename;
    this.currentSpriteSheetIMAGE = new Image();
    this.currentSpriteSheetIMAGE.onload = function() {
      _this.currentSpriteSheetOK = true;
      if (data.select) {
  ...
  getAnimationSpriteSheets: function(callback) {
    ...
    $.ajax({
      type: "POST",
      url: '/jaajCommand',
      data: 'getAnimationSpriteSheets:'+CurrentJaaJWorld.name,
      processData: false,
      contentType: false,
      success: function(data) {
        if (!data) {
  ...
  getItemImage: function(callback) {
    ...
    $.ajax({
      type: "POST",
      url: '/jaajCommand',
      data: 'getItemImage:'+CurrentJaaJWorld.name,
      processData: false,
      contentType: false,
      success: function(data) {
        if (!data) {
  ...
  getTilesets: function(callback) {
    ...
    $.ajax({
      type: "POST",
      url: '/jaajCommand',
      data: 'getTilesets:'+CurrentJaaJWorld.name,
      processData: false,
      contentType: false,
      success: function(data) {
                ...
                onopen: function(id) {
                  for (var i in files) {
                    if (files.hasOwnProperty(i)) {
                      $('#'+id).append('<div class="UI-Create_SelectSpriteSheet-ContainerImage" onclick="ui.window.close(\''+id.replace(/^WINDOW_/g,'').replace(/_Body$/g,'')+'\');resource.setSpriteSheet(\''+files[i]+'\');"><div class="UI-Create_SelectSpriteSheet-Image" style="background: url(assets/map/'+CurrentJaaJWorld.name+'/mob/'+files[i]+') center no-repeat; background-size: contain;"></div></div>');
                    }
                  }
                }
              });
            ...
            resource.getAnimationSpriteSheets(function(files){
              ui.window.open({
                ...
                onopen: function(id) {
                  for (var i in files) {
                    if (files.hasOwnProperty(i)) {
                      $('#'+id).append('<div class="UI-Create_SelectSpriteSheet-ContainerImage" onclick="ui.window.close(\''+id.replace(/^WINDOW_/g,'').replace(/_Body$/g,'')+'\');resource.setAnimationSpriteSheet(\''+files[i]+'\');"><div class="UI-Create_SelectSpriteSheet-Image" style="background: url(assets/map/'+CurrentJaaJWorld.name+'/animation/'+files[i]+') center no-repeat; background-size: contain;"></div></div>');
                    }
                  }
                }
              });
          ...
          if (typeof __this.id != 'undefined') {
            if (__this.name != '') {
              if (__this.id == null) {
                ...
                var srcImage = $('#UI-ImageItem').attr('src') || '';
                __this.data.Image = srcImage.length > 1 ? srcImage.substring(srcImage.lastIndexOf('/')+1) : srcImage;
                ...
                var srcImage = $('#UI-ImageItem').attr('src') || '';
                console.log(srcImage.substring(srcImage.lastIndexOf('/')+1));
                __this.data.Image = srcImage.length > 1 ? srcImage.substring(srcImage.lastIndexOf('/')+1) : srcImage;
                d.row['data'] = JSON.stringify(Object.assign({},__this.data));
                socket.SetMysql(d,function(data){
                  new toolTip('<span style="color: #0f0;">Item: Success Save!</span>',4000);
            ...
            resource.getItemImage(function(files){
              ui.window.open({
                ...
                onopen: function(id) {
                  for (var i in files) {
                    if (files.hasOwnProperty(i)) {
                      $('#'+id).append('<div class="UI-Create_SelectSpriteSheet-ContainerImage" onclick="ui.window.close(\''+id.replace(/^WINDOW_/g,'').replace(/_Body$/g,'')+'\');$(\'#UI-ImageItem\').attr(\'src\',\'assets/map/'+CurrentJaaJWorld.name+'/item/'+files[i]+'\');"><div class="UI-Create_SelectSpriteSheet-Image" style="background: url(assets/map/'+CurrentJaaJWorld.name+'/item/'+files[i]+') center no-repeat; background-size: contain;"></div></div>');
                    }
                  }
                }
              });
        ...
        setHTML: function(_this,data) {
          ...
          */
          ...
          $('#'+_this.idBody).append(
            ...
            '<img class="UI-Create-ImageJaaj" src="assets/map/'+CurrentJaaJWorld.name+'/jaajs/'+src+'" />'
          );
          __this.startEvent(_this);
        },
        startEvent: function(_this) {
            ...
            resource.getJaajs(function(files){
              ui.window.open({
                ...
                onopen: function(id) {
                  for (var i in files) {
                    if (files.hasOwnProperty(i)) {
                      $('#'+id).append('<div class="UI-Create_SelectSpriteSheet-ContainerImage" onclick="ui.window.close(\''+id.replace(/^WINDOW_/g,'').replace(/_Body$/g,'')+'\');$(\'.UI-Create-ImageJaaj\').attr(\'src\',\'assets/map/'+CurrentJaaJWorld.name+'/jaajs/'+files[i]+'\');resource.jaajSrc = \''+files[i]+'\';"><div class="UI-Create_SelectSpriteSheet-Image" style="background: url(assets/map/'+CurrentJaaJWorld.name+'/jaajs/'+files[i]+') center no-repeat; background-size: contain;"></div></div>');
                    }
                  }
                }
              });
    ...
    Jaajs_List: {
      modelHTML: function(param) {
        ...
        if (imagePath == '') {
          ...
          return '<div class="UI-Create_Jaaj" data-id="'+param.id+'" style="border: 1px solid #ff0; background: url(assets/map/'+CurrentJaaJWorld.name+'/jaajs/'+imagePath+') no-repeat center; background-size: contain;"></div>'
        }
      },
      setHTML: function(_this) {
        var __this = this;
...
Editor.prototype = {
    ...
    for (var i = 0; i < listZone.length; i++) {
      for (var j = 0; j < dataName.length; j++) {
        if (typeof window[dataName[j].toLowerCase()].data[listZone[i]] == 'undefined') {
          listFileJSON.push({name:listZone[i]+'_'+dataName[j]+'.json',JaaJWorld:CurrentJaaJWorld.name,area:listZone[i]});
  ...
  get: function(settings) {
    ...
    for (var i = 0; i < settings.data.length; i++) {
      (function(i){
        ...
        xhrl[i].send(JSON.stringify([{name:settings.data[i].name,JaaJWorld:CurrentJaaJWorld.name}]));
      })(i);
    }
  },
  ...
  save: function(area,c) {
    if (!this.inUse) {
      ...
      this.dataList[0] = '{"area":"'+area+'","JaaJWorld":"'+CurrentJaaJWorld.name+'","name":"Resource","extension":"json","data":'+this.dataList[0]+'}';
...
function JaaJGame(JaaJWorldData,directlyChange) {
  CurrentJaaJWorld = JaaJWorldData;
  console.clear();
  upDateLoop = false;
  canvas = null,
  ctx = null,
  ...
  Key._pressed = {};
  if (directlyChange) {
    create(CurrentJaaJWorld);
  } else {
    return getQueryString(CurrentJaaJWorld);
  }
};

function create(JaaJWorldData) {
  JaaJWorldData = JaaJWorldData || {};
  if (typeof JaaJWorldData.name !== 'undefined' && typeof JaaJWorldData.editor !== 'undefined') {
    CurrentJaaJWorld = JaaJWorldData;
  } else if ($_GET('name')) {
    CurrentJaaJWorld = $_GET();
  }
  ...
  //if (checkCookie('JAAJeditorUsername')) {
    ...
    editor = new Editor();
    player = new Player();
    socket = new Socket({mail:getCookie('AccJAAJeditorMail'),password:getCookie('AccJAAJeditorPassword'),name:getCookie('AccJAAJeditorUsername'),color:getCookie('AccJAAJeditorColor')});
    maker = new Maker();
    ...
    if (!CurrentJaaJWorld.editor) {
      $('#UI').css('display','none');
      $('#z').css('display','none');
      player.active(true,true);
    }
    if (!CurrentJaaJWorld.player) {
      CurrentJaaJWorld._fn();
    }
          ...
          (function(file){
            ...
            reader.onload = function(){
              ...
              if (name != '' && ext != '' && typeof listExt[ext] != 'undefined') {
                tilesetList.push({
                  name: name+'.'+ext,
                  JaaJWorld: CurrentJaaJWorld.name,
                  data: dataURL
                });
                nb++;
              }
            ...
            onopen: function(id) {
              ...
              for (var i = 0; i < tilesets.length; i++) {
                ...
                '<div class="UI-Tileset">'+
                  '<div class="UI-Tileset-Image" style="background: url(assets/map/'+CurrentJaaJWorld.name+'/tilesets/'+tilesets[i]+') no-repeat center; background-size: 100%;"></div>'+
              ...
              $('.UI-Tileset-Button-Select').click(function(){
                ...
                resource.currentTilesetURI = 'assets/map/'+CurrentJaaJWorld.name+'/tilesets/'+this.dataset.name;
    ...
    editor.load([
      ...
      player.memory = {"jaajs":[{"id":1,"evolutionLvl":0,"exp":0,"IV":[0,0,0],"EV":[0,0,0],"listAttack":[1]},{"name":"TITOUAN","id":1,"evolutionLvl":1,"exp":168240,"IV":[99999,0,0],"EV":[0,0,0],"listAttack":[1]},{"id":2,"evolutionLvl":0,"exp":33824,"IV":[0,0,0],"EV":[0,0,0],"listAttack":[1]}],"items":[],"klnct7n4qdeb9erssjorw0fj6udnbdwh9kck57b9":{"jaaj":{}}};
...
Socket.prototype = {
  connect: function(callback) {
    ...
    this.sk2 = io.connect('http://flammrock.ga/');
    this.sk.on('disconnect', this.disconnect);
    this.sk2.on('JAAJeditorOnConnect', this.onconnect);
    this.sk2.on('JAAJeditorOnDisconnect', this.ondisconnect);
    this.sk.on('JAAJeditorOnDataSend', this.ondatasend);
    this.sk.on('JAAJeditorOnSetMysql', this.onSetMysql);
    this.sk2.on('JAAJeditorOnSetMysql', this.onSetMysql2);
    this.sk2.on('JAAJeditorOnIsValidSession', this.isValidSession);
    this.sk2.emit('JAAJeditorConnect',{name:this.name+'#'+this.id,color:this.color});
    this.sk2.emit('JAAJeditorIsValidSession',{mail:this.mail,password:this.password});
    $('body').css('cursor','url(assets/map/'+CurrentJaaJWorld.name+'/editor/cursor/'+this.color+'.png), default');
    callback();
  },
  disconnect: function() {},
  ...
  SetMysql: function(data,c) {
    ...
    data.table = (CurrentJaaJWorld.name+'_'+data.table).toLowerCase();
    socket.sk.emit('JAAJeditorSetMysql',data);
  },
  onSetMysql2: function(data) {
    //error or success
    data = data || {};
    if (typeof data.error != 'undefined') {
      new toolTip('<span style="color: #f00;">Error Mysql!</span>',4000);
  ...
  SetMysql2: function(data,c) {
    ...
    socket.sk2.emit('JAAJeditorSetMysql',data);
  },
